...
public class S3FileSystemProvider extends FileSystemProvider {
    ...
    public FileSystem newFileSystem(final URI uri, final Map<String, ?> env) throws IOException {
        ...
        try (var client = new S3ClientProvider(config).configureCrtClient().build()) {
            ...
            if(envMap.containsKey("acl")) request.acl(envMap.get("acl").toString());
            if(envMap.containsKey("grantFullControl")) request.grantFullControl(envMap.get("grantFullControl").toString());
            if(envMap.containsKey("grantRead")) request.grantRead(envMap.get("grantRead").toString());
            if(envMap.containsKey("grantReadACP")) request.grantReadACP(envMap.get("grantReadACP").toString());
            if(envMap.containsKey("grantWrite")) request.grantWrite(envMap.get("grantWrite").toString());
            if(envMap.containsKey("grantWriteACP")) request.grantWriteACP(envMap.get("grantWriteACP").toString());
            if (envMap.containsKey("locationConstraint")) request.createBucketConfiguration(confBuilder -> {
